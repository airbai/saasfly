...
import { mapCustomChatLogProbsOutput } from './map-custom-chat-logprobs';
...
export class CustomChatLanguageModel implements LanguageModelV1 {
  ...
  private getArgs({
    ...
    const baseArgs = {
      ...
      // messages:
      messages: convertToCustomChatMessages({
        prompt,
  ...
  async doGenerate(
    ...
    return {
      ...
      warnings,
      logprobs: mapCustomChatLogProbsOutput(choice.logprobs),
    };
  }
      ...
      stream: response.pipeThrough(
        ...
        >({
          transform(chunk, controller) {
            ...
            const mappedLogprobs = mapCustomChatLogProbsOutput(
              choice?.logprobs,
            );
            ...
            if (mappedToolCalls != null) {
              for (const toolCallDelta of mappedToolCalls) {
                ...
                // Tool call start. Custom returns all information except the arguments in the first chunk.
...
const customTokenUsageSchema = z
  .object({
...
const customChatResponseSchema = z.object({
  ...
  usage: customTokenUsageSchema,
});

// limited version of the schema, focussed on what is needed for the implementation
// this approach limits breakages when the API changes and increases efficiency
const customChatChunkSchema = z.union([
  z.object({
    id: z.string().nullish(),
    created: z.number().nullish(),
    model: z.string().nullish(),
